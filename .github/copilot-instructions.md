...
- **Import grouping: standard library, then third-party, then local `app.*` imports, each group separated by a blank line** (PEP 8 §Imports), enforced in CI by `ruff check` (`I001`). Every file in `app/` and `tests/` already follows this — it wasn't always true (an earlier version of this file called out `api.py`/`models.py` as exceptions), but CI's isort-equivalent check now keeps it that way automatically.
- **Line length: keep new lines at or under 120 characters — CI enforces this with `ruff check` (`E501`), so a longer line fails the build, not just review.** PEP 8's canonical limit is 79; 120 is what actually holds across this codebase's real content, including descriptive test docstrings (99 was tried first and produced 71 violations, nearly all prose in test docstrings — 120 was the number that made the full tree clean without rewriting them). Prefer wrapping (parenthesized multi-line expressions, like the import block in `api.py`) over a long single line.
...
- **No trailing whitespace**, enforced in CI by `ruff check` (`W291`/`W293`). The original scaffold had 45 such lines across `api.py`/`storage.py`; all fixed, so the check now starts clean and stays that way.
