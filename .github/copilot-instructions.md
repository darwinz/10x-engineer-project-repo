...
- **Line length: keep new lines at or under 99 characters.** PEP 8's canonical limit is 79, but nothing in this repo enforces it and the longest existing line (`storage.py`) is 104 characters with no linter configured. 99 is the practical ceiling — don't add anything longer than what's already shipped, and prefer wrapping (parenthesized multi-line expressions, like the import block in `api.py`) over a long single line.
- **Double-quoted strings**, matching the dominant style across every file in `app/` (single quotes appear only a handful of times, generally where a string itself contains a `"`). Use single quotes only to avoid escaping a double quote inside the string.
- **f-strings for interpolation** (`f"{field} cannot be null"` in `api.py`), never `.format()` or `%`-formatting.
- **Full type hints on every function signature**, parameters and return type — every function in `app/` already has them; a new function without one is incomplete. Keep using `typing.Optional[X]` / `typing.List[X]` (as every existing file does) rather than the newer PEP 604/585 syntax (`X | None`, `list[X]`) — both are valid on this project's Python 3.10–3.12 floor, but mixing the two styles across the same small codebase is worse than picking the one already in use everywhere.
- **No wildcard imports** (`from x import *`) — none exist today; don't introduce one.
- **No trailing whitespace.** A handful of blank lines inside function bodies in `storage.py` currently have trailing spaces; that's pre-existing debt, not a pattern to match — new or touched lines should have none.
